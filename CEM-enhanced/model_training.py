...
# ============================================================================
# CEM-ENHANCED: 最强串行架构 + 多尺度特征 + 层次化注意力
# 核心创新：Attention→GMM串行 + 直接损失融合 + 多层特征增强
# ============================================================================

class EnhancedSlotAttention(nn.Module):
    """增强版Slot Attention：多层级联 + 残差连接 + 温度调节"""
    def __init__(self, feature_dim, num_slots=8, num_iterations=4):
        super().__init__()
        ...
        self.slot_mu = nn.Parameter(torch.randn(1, 1, feature_dim) * 0.02)
        ...
        # Enhanced update mechanism with residual connections
        ...
        # Slot competition mechanism
        self.slot_competition = nn.Sequential(
    ...
    def forward(self, inputs):
        ...
        # Enhanced slot initialization with better variance control
        mu = self.slot_mu.expand(batch_size, self.num_slots, -1)
        sigma = torch.clamp(self.slot_log_sigma.exp(), min=0.001, max=0.1)
        sigma = sigma.expand(batch_size, self.num_slots, -1)
        slots = mu + sigma * torch.randn_like(mu)
        
        # Iterative refinement with temperature annealing
        for iteration in range(self.num_iterations):
            slots_prev = slots
            
            # Temperature annealing for better convergence
            temperature = max(0.1, 1.0 - iteration * 0.2)
            ...
            # Scaled attention with temperature
            attn = torch.matmul(q, k.transpose(-2, -1)) / (self.feature_dim ** 0.5 * temperature)
            attn = F.softmax(attn, dim=-1)
            
            # Apply attention with residual connection
            updates = torch.matmul(attn, v)
            slots = self.norm1(slots_prev + updates)
            
            # Enhanced MLP with residual connection
            mlp_out = self.mlp(slots)
            slots = self.norm2(slots + mlp_out)
            
            # Slot competition to encourage diversity
            competition_weights = self.slot_competition(slots)
            slots = slots * competition_weights
            
        return slots

class SlotAttention(nn.Module):
    """原始Slot Attention保持兼容性"""
    def __init__(self, feature_dim, num_slots=8, num_iterations=3):
        super().__init__()
        self.enhanced_slot = EnhancedSlotAttention(feature_dim, num_slots, num_iterations)
        
    def forward(self, inputs):
        return self.enhanced_slot(inputs)

class EnhancedCrossAttention(nn.Module):
    """增强Cross Attention：多头 + 残差连接 + Layer Norm"""
    ...
    def forward(self, query_features, key_value_features):
        ...
        k = self.to_k(key_value_features).view(batch_size, -1, self.num_heads, self.head_dim).transpose(1, 2)
        v = self.to_v(key_value_features).view(batch_size, -1, self.num_heads, self.head_dim).transpose(1, 2)
        ...
        out = self.norm1(residual + self.to_out(out))
...
class CrossAttention(nn.Module):
    """原始CrossAttention保持兼容性"""
    def __init__(self, feature_dim, num_heads=4):
        super().__init__()
        self.enhanced_cross = EnhancedCrossAttention(feature_dim, num_heads * 2)  # 更多头数
        
    def forward(self, query_features, slot_outputs):
        return self.enhanced_cross(query_features, slot_outputs)
...
class SerialAttentionGMMCEM(nn.Module):
    """
    🚀 CEM-ENHANCED: 串行Attention→GMM架构 + 多尺度特征增强
    
    核心创新：
    1. 串行架构：Attention先提取高级特征，GMM再进行聚类优化
    2. 多尺度特征融合：不同层次的特征表示
    3. 直接损失融合：条件熵损失直接参与优化
    4. 自适应特征门控：动态调节特征重要性
    """
    def __init__(self, feature_dim=128, num_slots=12):  # 增加slot数量
        super().__init__()
        self.num_slots = num_slots
        self.feature_dim = feature_dim
        
        # 🚀 串行架构核心组件
        self.slot_attention = SlotAttention(feature_dim, num_slots, num_iterations=4)  # 更多迭代
        self.cross_attention = CrossAttention(feature_dim, num_heads=8)  # 更多头数
        ...
        self.feature_gate = nn.Sequential(
            nn.Linear(feature_dim * 3, feature_dim),  # 融合三个尺度
            nn.Sigmoid()
        )
        
        # 🚀 增强的自适应权重模块  
        self.adaptive_weight = EnhancedAdaptiveWeight(feature_dim)
    ...
    def forward(self, features, labels, unique_labels, centroids_list):
        """
        🚀 串行架构核心：Attention→GMM流水线 + 直接损失融合
        
        流程：
        1. 多尺度特征处理
        2. Slot Attention特征提取
        3. Cross Attention特征增强 
        4. 特征重构与门控
        5. GMM聚类优化
        6. 自适应权重融合
        """
        ...
        for processor in self.multi_scale_processor:
            scale_features = processor(features)
        ...
        # 步骤2：自适应特征门控
        gate_weights = self.feature_gate(concatenated_features)
        gated_features = features * gate_weights  # 原始特征 × 门控权重
        
        # 步骤3：串行Attention处理 - 这是核心创新！
        ...
        # 步骤5：GMM处理增强后的特征
        gmm_loss, gmm_mse = self.compute_enhanced_gmm_branch(
            reconstructed_features, labels, unique_labels, centroids_list
        )
        
        # 步骤6：直接损失融合（无需自适应权重，直接使用增强特征）
        return gmm_loss, gmm_mse
        
    def process_attention_serial(self, features, labels, unique_labels):
        """串行Attention处理：逐类别进行深度特征提取"""
        ...
        for i in unique_labels_np:
            ...
            # 🚀 串行处理：Slot Attention → Cross Attention
            # 第一步：Slot Attention提取类别特定的潜在表示
            slot_outputs = self.slot_attention(class_features)
            
            # 第二步：Cross Attention增强原始特征
            enhanced_class_features = []
            for j in range(class_features.shape[0]):
                enhanced_feat = self.cross_attention(
                    class_features[j:j+1], 
                    slot_outputs[j:j+1]
                )
                enhanced_class_features.append(enhanced_feat)
            
            if enhanced_class_features:
                enhanced_class_features = torch.cat(enhanced_class_features, dim=0)
                
                # 🚀 特征增强：残差连接 + Layer Norm
                enhanced_class_features = F.layer_norm(
                    class_features + enhanced_class_features,
                    normalized_shape=[feature_dim]
                )
                
                # 更新对应位置的特征
                enhanced_features[class_mask.squeeze(), :] = enhanced_class_features
    ...
    def compute_enhanced_gmm_branch(self, features, labels, unique_labels, centroids_list):
        ...
        for i in unique_labels_np:
            ...
            # 🚀 增强的距离计算：使用欧几里得距离 + 余弦相似度
            euclidean_distances = torch.cdist(class_features, centroids)
            cosine_similarities = F.cosine_similarity(
                class_features.unsqueeze(1), 
                centroids.unsqueeze(0), 
                dim=2
            )
            
            # 组合距离度量
            combined_distances = euclidean_distances - 0.1 * cosine_similarities
            ...
            for j in unique_cluster_assignments:
                ...
                weight = np.sum(indices_cluster) / class_features.shape[0]
                
                if np.sum(indices_cluster) > 0:
                    cluster_features = class_features[indices_cluster]
                    centroid = centroids[j]
                    
                    # 🚀 增强的方差计算：加入正则化项
                    variances = torch.mean((cluster_features - centroid)**2, dim=0)
                    reg_variances = variances + 0.0001  # 更小的正则化项
                    
                    if self.log_entropy == 1:
                        # 🚀 改进的互信息计算
                        mutual_info = F.relu(
                            torch.log(reg_variances + 1e-8) - torch.log(torch.tensor(0.0001))
                        )
                        weighted_variance = mutual_info.mean() * weight
                    else:
                        weighted_variance = reg_variances.mean() * weight
...
class EnhancedAdaptiveWeight(nn.Module):
    """增强的自适应权重模块"""
    def __init__(self, feature_dim):
        super().__init__()
        self.feature_dim = feature_dim
        ...
        # 动态权重调节
        self.dynamic_weight = nn.Parameter(torch.tensor(0.5))
    ...
    def forward(self, features, labels, unique_labels):
        ...
        for i in unique_labels_np:
            ...
            if class_features.shape[0] > 1:
                # 多维度复杂度计算
                var_complexity = torch.var(class_features, dim=0).mean()
                std_complexity = torch.std(class_features, dim=0).mean()
                range_complexity = (class_features.max(dim=0)[0] - class_features.min(dim=0)[0]).mean()
                
                combined_complexity = (var_complexity + std_complexity + range_complexity) / 3
                total_complexity += combined_complexity * class_features.shape[0]
                total_samples += class_features.shape[0]
        
        if total_samples > 0:
            avg_complexity = total_complexity / total_samples
            complexity_features = torch.cat([
                avg_complexity.unsqueeze(0).repeat(self.feature_dim // 4),
                torch.std(features, dim=0)[:self.feature_dim // 4],
                torch.mean(features, dim=0)[:self.feature_dim // 4],
                torch.var(features, dim=0)[:self.feature_dim // 4]
            ])
            
            base_weight = self.complexity_encoder(complexity_features)
            dynamic_factor = torch.sigmoid(self.dynamic_weight)
            
            final_weight = base_weight * dynamic_factor + (1 - dynamic_factor) * 0.5
            return torch.clamp(final_weight.item(), 0.1, 0.9)
        else:
            return 0.5
...
class HybridGMMAttentionCEM(nn.Module):
    ...
    def __init__(self, feature_dim=128, num_slots=8):
        super().__init__()
        self.serial_cem = SerialAttentionGMMCEM(feature_dim, num_slots)
        self.feature_dim = feature_dim
        self.log_entropy = 1
        
    def forward(self, features, labels, unique_labels, centroids_list):
        """直接调用串行增强架构"""
        return self.serial_cem(features, labels, unique_labels, centroids_list)
...
class MIA_train: # main class for every thing
    ...
    def compute_class_means(self, features, labels, unique_labels, centroids_list):
        """
        🚀 CEM-ENHANCED: 串行Attention→GMM架构 + 直接损失融合
        
        核心突破：
        1. 串行架构：Attention先深度提取特征，GMM再精确聚类
        2. 多尺度特征融合：不同粒度的特征表示增强
        3. 直接损失融合：条件熵损失直接参与总损失优化
        4. 自适应特征门控：动态调节特征重要性
        5. 增强距离度量：欧几里得距离 + 余弦相似度组合
        """
    ...
    def train_target_step(self, x_private, label_private, adding_noise,random_ini_centers,centroids_list,client_id=0):
        ...
        # 🚀 CEM-ENHANCED: 简化的反向传播
        # 条件熵损失已包含在total_loss中，使用统一的反向传播
        total_loss.backward()
