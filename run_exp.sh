...
use_attention_classifier="--use_attention_classifier"
num_slots=8
attention_heads=8
                ...
                for regularization_strength in $regularization_strength_list; do
                        for cutlayer in $cutlayer_list; do
                                for bottleneck_option in $bottleneck_option_list; do
                                        ...
                                        filename=pretrain_${pretrain}_lambd_${lambd}_noise_${regularization_strength}_epoch_${num_epochs}_bottleneck_${bottleneck_option}_log_${log_entropy}_ATstrength_${AT_regularization_strength}_lr_${learning_rate}_varthres_${var_threshold}_attention
...
########################### training the model ########################### please comments this part for model inversion attack  only
                                        if [ "$pretrain" = "True" ]; then
                                                ...
                                                ${use_attention_classifier} --num_slots ${num_slots} --attention_heads ${attention_heads} --attention_dropout ${attention_dropout}
                                        else
                                                num_epochs=240
                                                learning_rate=0.05
                                                CUDA_VISIBLE_DEVICES=${GPU_id} python main_MIA.py --arch=${arch}  --cutlayer=$cutlayer --batch_size=${batch_size} \
